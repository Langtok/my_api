Doorkeeper.configure do
  ...
  # Resource owner authenticator for Authorization Code grant flow
  resource_owner_authenticator do
    current_user || warden.authenticate!(scope: :user)
  end

  # Resource owner from credentials for Password grant flow
  resource_owner_from_credentials do |_routes|
    user = User.find_for_database_authentication(email: params[:username])
    if user&.valid_password?(params[:password])
      user
    else
      # Optional: Add error logging or return specific error message
      nil
    end
  end

  # Enable specific grant flows
  grant_flows %w[password client_credentials]

  # Client credentials configuration
  client_credentials do
    # Example: Configure the expiration time for client credentials tokens
